...
contract("ComposableTopDown", function (accounts) {
  ...
  it("should mint a 721 token, Composable", async () => {
    ...
    assert(
      tokenId.toNumber() == 1,
      "Composable 721 token was not created or has wrong tokenId"
    );
  ...
  it("should mint a 721 token, SampleNFT", async () => {
    ...
    assert(
      tokenId.toNumber() == 0,
      "SampleNFT 721 token was not created or has wrong tokenId"
    );
  ...
  it("should safeTransferFrom SampleNFT to Composable", async () => {
    ...
    // const tx = await sampleNFT.contract.safeTransferFrom[
    //   "address,address,uint256,bytes"
    // ](alice, composable.address, 1, bytes1, { from: alice, gas: 500000 });

    const tx = await sampleNFT.safeTransferFrom(
      alice,
      composable.address,
      0,
      bytes1
  ...
  it("should own sampleNFT, Composable", async () => {
    const owned = await composable.childExists(sampleNFT.address, 0);
    assert(owned, "composable does not own sampleNFT");
  });

  it("should get owning token of Composable", async () => {
    const result = await composable.ownerOfChild(sampleNFT.address, 0);
    //console.log(result);
    //console.log("tokenID:"+tokenId);
    assert(result[1].toNumber() == 1, "composable parent not found");
  });
  ...
  it("should have 1 child contract address sampleNFT", async () => {
    ...
    const tokenId = await composable.childTokenByIndex.call(
      1,
      sampleNFT.address,
      0
    );
    ...
    assert(
      tokenId.toNumber() == 0,
      "call to composable.childTokenByIndex failed or was wrong."
    );

    assert(
      contracts.toNumber() === 1 && contract === SampleNFT.address,
      "composable does not have the right childs contract"
    );
  });
  ...
  it("should transfer child to alice", async () => {
    const tx = await composable.transferChild(1, alice, sampleNFT.address, 0, {
      from: bob,
    });
    // const tx = await composable.contract.transferChild[
    //   "uint256,address,address,uint256"
    // ](1, alice, sampleNFT.address, 1, { from: bob, gas: 500000 });
    assert(tx, "Transaction undefined");
  });

  it("should own sampleNFT, alice", async () => {
    const address = await sampleNFT.ownerOf.call(0);
  ...
  //     assert(
  //       contracts.toNumber() == 0,
  //       "composable has wrong number of child contracts"
  //     );
  //   });
  ...
  it('should safeTransferFrom SampleNFT "1" to Composable "2"', async () => {
    // const tx = await sampleNFT.contract.safeTransferFrom[
    //   "address,address,uint256,bytes"
    // ](alice, composable.address, 2, bytes2, { from: alice, gas: 500000 });
  ...
  it("should have sampleNFT contract", async () => {
    ...
    assert(
      contract === sampleNFT.address,
      "composable does not have sampleNFT contract"
    );
  });

  it('should own sampleNFT "1", Composable "2"', async () => {
    const owned = await composable.childExists(sampleNFT.address, 1);
    assert(owned, "composable does not own sampleNFT 1");
  });
  ...
  it("should safeTransferChild from composable 2 to composable 1", async () => {
    ...
    const tx = await composable.safeTransferChild(
      2,
      composable.address,
      sampleNFT.address,
      1,
    ...
    // const safeTransferChild = ComposableTopDown.abi.filter(
    //   (f) => f.name === "safeTransferChild" && f.inputs.length === 5
    // )[0];
    // const transferMethodTransactionData = web3Abi.encodeFunctionCall(
    //   safeTransferChild,
    //   [2, composable.address, sampleNFT.address, 2, bytes1]
    // );
    // const tx = await web3.eth.sendTransaction({
    //   from: alice,
    //   to: composable.address,
  ...
  it("should have 1 child contract address sampleNFT", async () => {
    ...
    assert(
      contract === SampleNFT.address,
      "testing example failed, wrong contract address"
    );
  ...
  it("should own sampleNFT 1, composable", async () => {
    const address = await sampleNFT.ownerOf.call(1);
    assert(
      address == composable.address,
      "composable does NOT own sampleNFT 1"
    );
  });

  it("token 1 should own SampleNFT child token 1", async () => {
    const result = await composable.ownerOfChild.call(SampleNFT.address, 1);
    const tokenId = result[1];
    //console.log(tokenId);
    assert(
      tokenId.toNumber() == 1,
      "SampleNFT child token 2 is not owned by a composable token."
    );
  });

  it('should have 1 child of type sampleNFT of ID "1"', async () => {
    const childs = await composable.totalChildTokens.call(2, sampleNFT.address);
    assert(childs.toNumber() == 1, "composable does not own right childs");
  });
  ...
  it("should have 1 child contract addresses: SampleNFT", async () => {
    ...
    assert(
      contract === sampleNFT.address,
      "composable does not have a sampleNFT contract"
    );
  });
  /*
  ...
  */
  ...
  it("should have an ERC20 balance", async () => {
    ...
    assert(balance.toNumber() == 1000, "incorrect balance");
  });

  it('should transfer half the value from the ERC20 to the composable "3"', async () => {
    // const tx = await sampleERC20.transfer(composable.address, 500, bytes3);

    await sampleERC20.approve(composable.address, 500);
    const tx = await composable.getERC20(alice, 3, sampleERC20.address, 500);
    //   const bal = await composable.balanceOfERC20(3, sampleERC20.address);
    //   expect(bal.toNumber()).to.equal(10);

    // const transfer = SampleERC20.abi.filter(
    //   (f) => f.name === "transfer" && f.inputs.length === 3
    // )[0];
    // const transferMethodTransactionData = web3Abi.encodeFunctionCall(transfer, [
    //   composable.address,
    //   500,
    //   bytes3,
    // ]);
  ...
  it('should one contract in composable "3"', async () => {
    ...
    assert(
      contracts.toNumber() == 1,
      "ERC20 balance of composable NOT correct"
    );
  });

  it('should have half the balance of sampleERC20 in composable "3"', async () => {
    const balance = await composable.balanceOfERC20.call(
      3,
      sampleERC20.address
    );
    assert(
  ...
  it('should transfer half the balance in composable "3" to bob', async () => {
    ...
    const tx = await composable.transferERC20(3, bob, sampleERC20.address, 250);

    // const transfer = ComposableTopDown.abi.filter(
    //   (f) => f.name === "transferERC20" && f.inputs.length === 4
    // )[0];
    // const transferMethodTransactionData = web3Abi.encodeFunctionCall(transfer, [
    //   3,
    //   bob,
  ...
  it("should return the correct number of totalTokenContracts and balances after ERC20 contracts are added", async () => {
    ...
    assert(
      composableOwnerId === alice,
      `Alice does not own own composable token ${composableOwnerId}`
    );
    const numTokens = 5;
    ...
    for (var i = 0; i < erc20s.length; i++) {
      ...
      await erc20s[i].approve(composable.address, transferedAmount);
      const tx = await composable.getERC20(
        alice,
        2,
        erc20s[i].address,
        transferedAmount
      ...
      //   const transfer = erc20s[i].abi.filter(
      //     (f) => f.name === "transfer" && f.inputs.length === 3
      //   )[0];
      //   const transferMethodTransactionData = web3Abi.encodeFunctionCall(
      //     transfer,
      //     [composable.address, transferedAmount, bytes2]
      //   );
      //   const tx = await web3.eth.sendTransaction({
      //     from: alice,
      //     to: erc20s[i].address,
      ...
      //   });
      const balance = await composable.balanceOfERC20.call(
        aliceComposableTokenId,
        erc20s[i].address
      );
      assert(
        balance.toNumber() == transferedAmount,
        `Expected token #${i} to have a balance of ${transferedAmount} but got ${balance}`
      );
    }
    const numAddedTokenContracts = await composable.totalERC20Contracts(
      aliceComposableTokenId
    );
    assert(
      numAddedTokenContracts.toNumber() == numTokens,
  ...
  it("should return the correct number of totalChildTokens and totalChildTokens after NFT contracts are added", async () => {
    ...
    for (var i = 0; i < nfts.length; i++) {
      const mintedTokens = [];
      //   const safeTransferFrom = nfts[i].abi.filter(
      //     (f) => f.name === "safeTransferFrom" && f.inputs.length === 4
      //   )[0];
      for (var j = 0; j < mintedPerNFT; j++) {
        await nfts[i].mint721(alice);
        const mintedTokenId = j;
        mintedTokens.push(mintedTokenId);
        const tx = await nfts[i].safeTransferFrom(
          alice,
          composable.address,
          mintedTokenId,
          bytes2
        ...
        // const transferMethodTransactionData = web3Abi.encodeFunctionCall(
        //   safeTransferFrom,
        //   [alice, composable.address, mintedTokenId, bytes2]
        // );
        // const tx = await web3.eth.sendTransaction({
        //   from: alice,
        //   to: nfts[i].address,
      ...
      const numNFTChildren = await composable.totalChildTokens(
        aliceComposableTokenId,
        nfts[i].address
      );
      assert(
        numNFTChildren.toNumber() == mintedTokens.length,
        `Expected number of totalChildTokens to be ${mintedTokens.length} but got ${numNFTChildren}`
      );
    }
    const totalContracts = await composable.totalChildContracts(
      aliceComposableTokenId
    );
    assert(
      totalContracts.toNumber() == numNFTs + 1,
      `Expected totalChildContracts to be ${numNFTs} but got ${totalContracts}`
    );
  });

  xit("should return the correct number of totalTokenContracts after ERC20 contracts are removed", async () => {
    const aliceComposableTokenId = 2;
    let numTokenContracts = (
      await composable.totalERC20Contracts(1)
    ).toNumber();
    let nextNumTokenContracts;
    for (var i = 0; i < numTokenContracts; i++) {
      const tokenAddress = await composable.tokenContractByIndex(
        aliceComposableTokenId,
        i
      );
      const remainingBalance = await composable.balanceOfToken.call(
        aliceComposableTokenId,
        tokenAddress
      );
      const transferToken = Composable.abi.filter(
        (f) => f.name === "transferToken" && f.inputs.length === 4
      )[0];
      const transferMethodTransactionData = web3Abi.encodeFunctionCall(
        transferToken,
        [
      ...
      nextNumTokenContracts = (
        await composable.totalTokenContracts(1)
      ).toNumber();
      assert(
        numTokenContracts - 1 === nextNumTokenContracts,
        `Expected ${
          numTokenContracts - 1
        } tokenContracts but got ${nextNumTokenContracts}`
      );
  ...
  xit("should return the correct number of totalChildTokens and totalChildContracts when NFT contracts are removed", async () => {
    const aliceComposableTokenId = 2;
    let totalChildContracts = await composable.totalChildContracts(
      aliceComposableTokenId
    );
    let numTokensToRemove = totalChildContracts;
    let nextTotalChildContracts;
    for (var i = 0; i < numTokensToRemove; i++) {
      const contractAddress = await composable.childContractByIndex(
        aliceComposableTokenId,
        totalChildContracts - 1
      );
      let totalChildTokens = await composable.totalChildTokens(
        aliceComposableTokenId,
        contractAddress
      );
      ...
      for (var j = 0; j < numChildTokensToRemove; j++) {
        const childTokenId = await composable.childTokenByIndex(
          aliceComposableTokenId,
          contractAddress,
          totalChildTokens - 1
        );
        const safeTransferChild = ComposableTopDown.abi.filter(
          (f) => f.name === "safeTransferChild" && f.inputs.length === 4
        )[0];
        const transferMethodTransactionData = web3Abi.encodeFunctionCall(
          safeTransferChild,
          [
        ...
        nextTotalChildTokens = await composable.totalChildTokens(
          aliceComposableTokenId,
          contractAddress
        );
        assert(
          nextTotalChildTokens.equals(totalChildTokens - 1),
          `Expected totalChildTokens to be ${
            totalChildTokens - 1
          } after removing contract ${i} at address ${contractAddress} but got ${nextTotalChildTokens}`
        );
        totalChildTokens = nextTotalChildTokens;
      }
      nextTotalChildContracts = await composable.totalChildContracts(
        aliceComposableTokenId
      );
      assert(
        nextTotalChildContracts.equals(totalChildContracts - 1),
        `Expected totalChildContracts to be ${
          totalChildContracts - 1
        } after removing tokens for contract ${contractAddress} but got ${nextTotalChildContracts}`
      );
